...
{if ($listpages[ix].size > 0) or (($prefs.feature_article_comments eq 'y') and ($tiki_p_read_comments eq 'y'))}
  {if ($listpages[ix].heading_only ne 'y')}
    {if ($listpages[ix].size > 0)}
	    ...
	    <a href="tiki-read_article.php?articleId={$listpages[ix].articleId}" class="trailer" id="article_read_more">{tr}Read More{/tr}</a>
	    </td>
  ...
  {if ($prefs.feature_article_comments eq 'y')
   ...
   and ($listpages[ix].allow_comments eq 'y')}
    <td class="articletrailer" id="article_comments">
...
{if $prefs.feature_cms_print eq 'y'}
  <a class="trailer" id="article_print" href="tiki-print_article.php?articleId={$listpages[ix].articleId}">{icon _id='printer' alt='{tr}Print{/tr}'}</a>
{/if}
