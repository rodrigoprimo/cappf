...
<form class="forms" method="get" action="tiki-searchindex.php">
   {* {tr}Find{/tr} *}<input id="fuser" class="search_top" name="highlight" size="14" type="text" accesskey="s" value="{$words}"/>
{if ( $searchStyle eq "menu" )}
    {* {tr}in{/tr}
    <select name="where">
	{if empty($where_list)}
...
{/if}
    <input id="busca" type="image" src="styles/cappf/busca.png" class="wikiaction" name="search" />
</form>
