...
{/if}
{if !$popup}
<img src="show_image.php?id={$imageId}&amp;scalesize={$resultscale}&amp;nocount=y" alt="{tr}image{/tr}" id="thepix" width="550"/>
{else}
<img src="show_image.php?id={$imageId}&amp;scalesize={$resultscale}&amp;nocount=y" alt="{tr}image{/tr}" id="thepix" width="900"/>
{/if}
{if $scaleinfo.clickscale >= 0}
</a>
{/if}
...
{if $popup eq ""}
  ...
  <table class="normal noslideshow">
	...
	{if $prefs.feature_maps eq 'y'}
  		<tr><td class="odd">{tr}Latitude (WGS84/decimal degrees){/tr}:</td><td class="odd">{$prefs.lat|escape}</td></tr>
  		<tr><td class="even">{tr}Longitude (WGS84/decimal degrees){/tr}:</td><td class="even">{$prefs.lon|escape}</td></tr>
  	{/if}
